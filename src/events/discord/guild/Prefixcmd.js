...
export default {
  ...
  async execute({ eventArgs, client }) {
    ...
    // ── Feedback: handle pending image uploads (works in guild + DM) ──────────
    const pending = pendingImageMap.get(message.author.id);
    if (pending) {
      const attachment = message.attachments.first();
      // Accept image attachments; fall back to filename extension when contentType is absent
      ...
      if (isImage) {
        ...
        try {
          ...
          await postFeedback({
            ...
            user: message.author,
            editFn: pending.editFn,
          });
        } catch (err) {
          logger.error("MessageCreate", "Failed to post feedback with image", err);
        }
      } else {
        // Not an image — remind the user without swallowing commands
        await message.reply(
          "📎 Drop an image to attach it to your review, or click **Skip** to post without one.",
        ).catch(() => {});
      }
      // Consume the message so it isn't processed as a command either way
      return;
    }
    // ─────────────────────────────────────────────────────────────────────────
    ...
    if (receiptPending) {
      ...
      if (isImage) {
        clearTimeout(receiptPending.timeout);
        ...
        await message.reply(
          "📸 Drop a screenshot — a receipt image is **required** to proceed. Or click **Cancel** to stop.",
        ).catch(() => {});
      }
      return;
    }
