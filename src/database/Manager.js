...
import { Guild, Panel, Ticket, ReceiptLog } from "./Schema.js";
...
export class DatabaseManager extends EventEmitter {
  ...
  async setReceiptChannel(guildId, channelId) {
    await this.createGuild(guildId);
    return await Guild.findOneAndUpdate(
      { guildId },
      { $set: { receiptChannel: channelId } },
      { returnDocument: 'after' }
    );
  }

  async getReceiptChannel(guildId) {
    const guild = await this.getGuild(guildId);
    return guild?.receiptChannel ?? null;
  }
  ...
  async getReceiptLog(messageId) {
    return await ReceiptLog.findOne({ messageId });
  }

  async actionReceiptLog(messageId, status, actionedBy) {
    return await ReceiptLog.findOneAndUpdate(
